<template>
  <div>
    <v-container>
      ...
      <v-btn block class="my-btn py-sm-5 title rounded-xl" @click="dialog = true">
        Підключитись
      </v-btn>
        ...
        <v-card>
          ...
          <v-container>
            ...
            <v-form lazy-validation ref="form">
              ...
              <v-text-field prepend-icon="mdi-account-edit" label="Ім'я" v-model="user_name" :rules="inputRules"
                            placeholder="Ім'я, По Батькові" renquired></v-text-field>
              <v-text-field prepend-icon="mdi-phone-in-talk" label="Телефон" v-model="user_phone" :rules="inputRules"
                            placeholder="+380" required></v-text-field>
              <v-select
              ...
              <v-btn block @click="submitForm()" class="my-btn py-sm-5 title rounded-xl">
                Надіслати
              </v-btn>
    ...
    <v-container fluid grid-list-md class="text-center">
      ...
      <v-layout row wrap>
        <v-card-text style="color:#002244;">
          ...
          <v-btn v-for="(icon,index) in social" :key="index" class="mx-4" style="color:#002244;" icon :href="icon.link"
                 target="_blank">
            <v-icon size="24px">
              {{ icon.title }}
            </v-icon>
...
let send_message = new Audio(require('@/assets/music/success-sound-effect.mp3'))
export default {
  data: () => ({
    dialog: false,
    //csrf token
    csrf: '',
    inputRules: [
      v => v.length >= 3 || 'Не вистачає символів'
    ],
    user_name: '',
    user_phone: '',
    message: 'Підключення',
  ...
  methods: {
   ...
   mounted() {
    axios.get('https://mag.golden.net.ua/api/csrf/cookie')
      .then((response) => {
